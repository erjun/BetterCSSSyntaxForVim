...
" Maintainer:	erjun <993911924@qq.com>
" URL:		
...
syn keyword cssTagName abbr address area a b base bdo blockquote body br button caption cite code col colgroup dd del dfn div dl dt em fieldset form h1 h2 h3 h4 h5 h6 head hr html img i iframe img input ins kbd label legend li link map menu meta noscript ol optgroup option p param pre q samp script select small span strong style sub sup tbody td textarea tfoot th thead title tr ul var header footer nav canvas article aside audio command datalist details embed figcaption figure hgroup keygen mark meter output progress rp rt ruby section source summary time video table

syn match cssAttrSelector "[~|^$*]\?=\(\w\+\)*" contained
" common
syn keyword cssAttrKey hreflang media ping rel target type for rel src title lang class id style href 
" input
syn keyword cssAttrKey accept autocomplete autofocus checked disabled form inputmode list max maxlength min name pattern required readonly replace step template value
" form
syn keyword cssAttrKey action data replace accept accept-charset enctype method
syn region cssAttributeSelector matchgroup=cssAttrSelector start="\[" end="]" transparent contains=cssUnicodeEscape,cssStringQ,cssStringQQ,cssAttrKey,cssAttrSelector
...
syn match cssBoxProp contained "\<\(width\|z-index\|opacity\)\>"
...
"At-Rules 
syn region cssRules start="@\(import\|charset\|fontdef url\)" end=";" contains=ALL
syn match cssRules "@page\>" nextgroup=cssDefinition
syn match cssRules "@font-face\>" nextgroup=cssFontDescriptorBlock skipwhite skipnl
syn match cssRules "@media\>" nextgroup=cssMediaType skipwhite skipnl
...
syn match cssError contained "{@<>"
 "{}
syn region cssDefinition transparent matchgroup=cssBraces start='{' end='}' contains=css.*Attr,css.*Prop,cssComment,cssValue.*,cssColor,cssURL,cssImportant,cssError,cssStringQ,cssStringQQ,cssFunction,cssUnicodeEscape,cssPrefix

" PseudoClass  TODO
syn match cssPseudoClass ":\S*" contains=ALL
...
if version >= 508 || !exists("did_css_syn_inits")
  ...
  HiLink cssAttrSelector Special
  HiLink cssPrefix Special
  HiLink cssFontProp StorageClass
  HiLink cssColorProp StorageClass
  HiLink cssTextProp StorageClass
  HiLink cssBoxProp StorageClass
  HiLink cssAttrKey StorageClass
